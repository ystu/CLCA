 ...
 */
public class CustomizedApriori {

	// the current level k in the breadth-first search
	protected int k; 

	// variables for statistics
	protected int totalCandidateCount; // number of candidate generated during last execution
	 ...
	 */
	public Itemsets runCustomizedApriori(double[] minsup, ArrayList<int[]> originalData, int longestLeng) throws IOException {
...
//		}
		patterns =  new Itemsets("frequent itemset of a batch");
		...
		// calculate minmum support
//		minsupRelative = minsup; 
		minsupRelative = new double[minsup.length];
		for(int i = 0; i<minsup.length ; i++){
			minsupRelative[i] = Math.ceil(minsup[i] * databaseSize);

		}
		
		long start_find_length_k_pattern = System.currentTimeMillis();
		...
		for(Entry<Integer, Integer> entry : mapItemCount.entrySet()){
			if(entry.getValue() >= minsupRelative[k]){
...
//					System.out.println("test");
	 ...
	 loopCand:	for(Itemset candidate : candidatesK){
			...
			for (Itemset candidate : candidatesK) {
				...
				if (candidate.getAbsoluteSupport() >= minsupRelative[k]) {
					// add the candidate
					level.add(candidate);
